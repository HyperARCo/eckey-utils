...
/**
 * @typedef {Object} PemResult
 * @property {string} privateKey
 * @property {string} publicKey
 */

/**
 * @typedef {Object} JWKKey
 * @property {"EC"} kty
 * @property {string} crv
...
/**
 * @typedef {Object} _JWKPrivateKey
 * @property {string} x
 * @property {string} y
 * @property {string} d
 * 
 * @typedef {JWKKey & _JWKPrivateKey} JWKPrivateKey
 */

/**
 * @typedef {Object} _JWKPublicKey
 * @property {string} x
 * @property {string} y
 * 
 * @typedef {JWKKey & _JWKPublicKey} JWKPublicKey
 */
...
/**
 * @param {Aio | string} arg1
 * @param {KeyPair} [arg2]
 * @returns {DerResult}
 *
 * @throws {Error} If curve is not defined
 * @throws {Error} If curve is not supported
 * @throws {Error} If neither public key nor private key are defined
 * @throws {Error} If either public key or private key is not a Buffer
 */
function generateDer(arg1, arg2) {
  ...
  if (!sk && !pk) throw Error("Either privateKey or publicKey is required");
 ...
 * @returns {KeyObj}
 */
function parseSkDer(der) {
  return parseDer$Ecsk(der);
}
 ...
 * @returns  {KeyObj}
 */
function parsePkDer(der) {
  return parseDer$Spki(der);
}

/**
 * @overload
 * @param {Aio} arg1 All in one config
 * @returns {PemResult}
 */

/**
 * @overload
 * @param {string} arg1 curve name
 * @param {KeyPair} arg2
 * @returns {PemResult}
 */
 ...
 * @throws {Error} If curve is not defined
 * @throws {Error} If curve is not supported
 * @throws {Error} If neither public key nor private key are specified
 * @throws {Error} If either public key or private key is not a Buffer
 */
function generatePem(arg1, arg2) {
  let { cname, sk, pk } = parseArgs(arg1, arg2);
  if (!cname) throw Error("Curve name is not optional");
  let crve = curveToOid[cname];
  if (!crve) throw Error("Unsupported elliptic curve");
 ...
 * @returns {KeyObj}
 *
 * @throws {Error} If pemContent is falsy
 * @throws {Error} If pemContent is invalid
 * @throws {Error} If pemContent is unsupported
 */
 ...
 * @returns {JWKResult}
 *
 * @throws {Error} If curve is outside of P-256/384/521
 * @throws {Error} If public key is not 'uncompressed' format
 */
function generateJwk(arg1, arg2) {
  let { cname, sk, pk } = parseArgs(arg1, arg2);
  ...
  if (sk && pk) {
    ...
    if (pk[0] !== 0x04) throw Error("Only support uncompressed public key");
 ...
 * @returns {KeyObj}
 *
 * @throws {Error} If jwk.kty is not 'EC'
 * @throws {Error} If jwk.crv is outside of P-256/384/521
 */
